...
class lambertian : public material {
    ...
    public :
        ...
        bool scatter(const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const override {
            ...
            if(scatter_direction.near_zero()){ // if by chance our rand unit vector is opposite of normal we just set to normal
...
class dielectric : public material {
    ...
    public:
        ...
        bool scatter(const ray& r_in, const hit_record& rec, color& attenuation, ray& scattered) const override {
            ...
            vec3 direction;
            if(ri * sin_theta > 1.0 || reflectance(cos_theta, ri) > random_double()) { // must reflect
                direction = reflect(unit_dir, rec.normal);
            }
            else { // allowed to refract
